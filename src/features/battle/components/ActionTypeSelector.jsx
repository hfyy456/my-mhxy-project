...
const ActionTypeSelector = ({ 
  selectedUnit, 
  selectedAction, 
  availableActions = ['attack', 'defend', 'skill', 'item', 'escape'],
  onActionSelect,
  disabled = false,
  className = ''
}) => {
  ...
  if (!selectedUnit) {
    return (
      <div className={`w-full h-full bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-lg border border-white/10 p-4 shadow-lg text-white font-sans flex flex-col items-center justify-center ${className}`}>
  ...
  return (
    <div className={`w-full h-full bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-lg border border-white/10 p-2 shadow-lg text-white font-sans flex flex-col ${className}`}>
      {/* 标题区域 */}
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <div className="text-xs text-gray-300">
              HP: {selectedUnit.stats?.currentHp || 0}/{selectedUnit.stats?.maxHp || 0} | 
              MP: {selectedUnit.stats?.currentMp || 0}/{selectedUnit.stats?.maxMp || 0}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-1 overflow-y-auto custom-scrollbar">
        {availableActions.map((actionType) => {
          ...
          return (
            <button 
              key={actionType}
              className={`px-3 py-1.5 rounded-lg font-medium text-xs transition-all duration-300 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-offset-gray-800 shadow-sm ${getButtonStyle(actionType)}`}
