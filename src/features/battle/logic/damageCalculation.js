 ...
 * @LastEditTime: 2025-06-13 02:39:57
...
export const calculateBattleDamage = (
  ...
  if (damageType === "physical") {
    ...
    console.log(
      `物理伤害计算: 攻击=${physicalAttack}, 防御=${physicalDefense}`
    );
    ...
    const magicalAttack =
      attackerStats.magicalAttack || attackerStats.mAtk || 0;
  ...
  //防御行动硬减伤，在所有伤害计算完之后再应用
  const defendReduction = 0.1;
  damageResult.finalDamage =
    Math.round(damageResult.finalDamage * (defender.isDefending ? 1 - defendReduction : 1));
  // 添加攻击者和防御者信息到结果中
  return {
    ...damageResult,
    attacker: {
