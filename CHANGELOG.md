...
- **新增技能 "火焰风暴"**:
  - 新增了目标为敌方全体的技能 "火焰风暴" (`fire_storm`)。
  - 为技能增加了专属的、居中于敌方阵地的 `fire_storm_aoe` 视觉特效，使用 `assets/skills/file_storm.gif` 文件。
  - 为施法者增加了独特的、朝向自适应的施法前摇动画 (`cast_step_forward`)，以区别于普通攻击。

### 🐛 错误修复 (Bug Fixes)

- **技能目标逻辑**:
  - 修复了群体和范围技能 (如 `group`, `area`) 错误地将目标固定为敌方阵营的问题，现在会根据施法者阵营正确选择目标。
  - 修复了范围技能 (如 `cross`) 因坐标属性名错误 (`r/c` 而非 `row/col`) 导致无法命中任何目标的问题。
  - 移除了 `skillExecution.js` 中多余且有缺陷的前置目标检查逻辑，该逻辑导致了范围技能无法正常执行。
- **动画与视觉**:
  - 修复了 `AnimationPlayer` 中 `SHOW_VFX` 指令只处理第一个目标的问题，现在可以为所有目标单位正确显示特效。
  - 修复了因 `BattleAnimations.css` 未被导入，导致新添加的 CSS 动画不生效的问题，已将样式整合进 `BattleSceneV3.jsx`。
  - 修复了特效无法显示的问题，为战斗场景添加了专门的 VFX 渲染层。
  - 为"火焰风暴"的受击单位增加了 `take_hit_shake` 晃动动画，并补全了其 CSS 定义。

### 💄 优化 (Refinements)

- **技能 "火焰十字斩"**:
  - 为 `fire_slash` 技能添加了完整的执行逻辑，包括对十字范围内所有目标的伤害计算和动画生成。
- **用户反馈**:
  - 将战斗中"无效目标"和"无有效目标"的模糊提示统一修改为更清晰的"没有存活的目标"。
- **特效定位**:
  - 将"火焰风暴"的全屏特效精确定位在敌方九宫格的中心，而非整个战场，视觉效果更合理。
  - 对"火焰风暴"的特效位置进行了微调，使其在视觉上更加协调。


---
## [0.4.3] - 2025-06-19 - 战斗体验优化

### 新增 (Added)

- **完整的治疗机制**:
  - 新增了治疗技能的后台逻辑，能够根据施法者的法术攻击力计算治疗量。
  - 实现了防止过量治疗的机制，确保生命值不会超过上限。
  - 伤害/治疗结算流程被统一，能同时处理正负生命值的变化。
  - 为治疗技能增加了全新的视觉效果，包括绿色的"+"HP浮动文字和单位脚下的"治疗光环"特效。

### 修复 (Fixed)

- **治疗逻辑Bug**: 修复了因属性名拼写错误 (`magicAttack` 应为 `magicalAttack`) 导致治疗技能完全失效的严重Bug。
- **动画系统**:
  - 修复了视觉特效（VFX）播放器中动画时长被硬编码为400ms的问题，现在不同特效（如攻击、治疗、防御）可以拥有各自独立的动画时长。
  - 修复了UI组件中因不当的状态突变（State Mutation）而导致治疗数字出现多个"+"号（例如 "++++256"）的显示错误。
- **视觉表现**:
  - 修复了部分浮动文字（如"生命已满"）因缺少 `color` 属性而无法正确显示颜色的问题。

---

## [0.4.2] - 2025-06-18 - 战斗体验优化

### ✨ 新增与优化 (Features & Improvements)

- **新增治疗技能体系:**
  - **技能逻辑实现:** 在战斗系统中加入了完整的`HEAL`技能类型处理逻辑，允许单位恢复生命值。
  - **友方目标选择:** 彻底重构了目标选择系统，现在玩家可以正确地为治疗或增益类技能选择友方单位。
  - **视觉效果区分:** 治疗数字现在会以醒目的**绿色**浮动文字显示，并自动附带 `+` 号，与红色的伤害数字明确区分。

- **嗜血追击 (Bloodthirsty Pursuit) 技能重做与动画优化:**
  - **连续追击动画:** 彻底重构了"嗜血追击"的攻击动画逻辑。攻击者现在会流畅地从一个目标冲向下一个目标，形成连贯的追击链，而不是每次都返回原位，大幅提升了技能的视觉表现力和打击感。
  - **精确的位移计算:** 解决了当攻击者从后排移动到前排时，位置计算不准确的问题。通过引入"起点"和"终点"的概念，动画系统现在可以精确计算点对点的移动向量，确保攻击者准确地移动到每个目标身前。
  - **统一视觉特效:** 根据最终设计，"嗜血追击"的所有攻击现在都统一使用更具表现力的 `bleed_effect`（红色斩击特效），视觉风格更加统一和凶猛。

### 🐛 错误修复 (Bug Fixes)

- **修复了治疗技能的目标错误:** 修正了一个严重错误，该错误曾导致治疗技能始终以施法者为目标，而不是玩家所选择的单位。
- **修复了过量治疗问题:** 为治疗逻辑增加了过量治疗检查，确保恢复的生命值不会超过目标的最大生命值上限。
- **修复了追击动画的移动向量:** 修正了`AnimationPlayer`中累积位移的计算错误，确保了连续攻击的路径和位置完全正确。
- **修复了动画时序计算错误:** 修正了在 `skillExecution.js` 中因错误地对常量赋值而导致的运行时崩溃问题，确保了复杂动画序列的延迟时间能够正确累加。
- **修复了视觉特效(VFX)的渲染:** 解决了由于错误的特效名称和缺失的动画定义导致特效无法显示的严重问题，确保 `bleed_effect` 能够被正确无误地渲染出来。


## [0.4.1] - 2025-06-18 - 战斗体验全方位升级
